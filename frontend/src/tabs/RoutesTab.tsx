...
import "../styles/route.css"; // 👈 Import CSS
...
export default function RoutesTab() {
  ...
  const filtered = routes.filter((r) => {
    ...
    if (filters.year && !String(r.year).includes(filters.year)) return false;
  ...
  return (
    <div className="routes-container">
      <h2 className="routes-title">Routes</h2>
        ...
        <tbody>
          {filtered.map((r) => (
            <tr key={r.routeId}>
              ...
              <td>{r.distance_km}</td>
              <td>{r.totalEmissions_t}</td>
              <td>{r.isBaseline ? "YES" : "NO"}</td>
              <td>
                <button onClick={() => setBaseline(r.routeId)}>
                  Set Baseline
                </button>
              </td>
            </tr>
